...
function SegmentedDateInput({
  ...
  function moveTo(seg: SegIdx, p: [string, string, string]) {
    ...
    announce(
      val
        ? `${DATE_SEG_STATIC[seg].label}: ${val}`
        : `Enter ${DATE_SEG_STATIC[seg].label}`
    );
  }
  ...
  function handleKeyDown(e: React.KeyboardEvent<HTMLInputElement>) {
    ...
    if (e.key === "Tab") {
      if (!e.shiftKey && seg < 2) {
        e.preventDefault();
        moveTo((seg + 1) as SegIdx, parts);
      } else if (e.shiftKey && seg > 0) {
        e.preventDefault();
        moveTo((seg - 1) as SegIdx, parts);
      }
      return;
    ...
    if (e.key === "ArrowLeft") {
      e.preventDefault();
      if (seg > 0) moveTo((seg - 1) as SegIdx, parts);
      return;
    }
    if (e.key === "ArrowRight") {
      e.preventDefault();
      if (seg < 2) moveTo((seg + 1) as SegIdx, parts);
      return;
    }
  ...
  return (
    <div className="relative">
      ...
      <span id={hintId} className="sr-only">
        Enter day, month, year. Use arrow keys to change values, Tab to move
        between segments, T for today, Escape to clear.
      </span>
    </div>
  );
}
