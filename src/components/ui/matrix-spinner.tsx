...
function buildDiags(): number[][] {
  ...
  for (let r = 0; r < 4; r++)
    for (let c = 0; c < 4; c++) d[c * 2 + r * 2].push(r * 4 + c);
  // offset: visual col = col*2+1, visual row = row*2+1
  for (let r = 0; r < 3; r++)
    for (let c = 0; c < 3; c++) d[c * 2 + 1 + (r * 2 + 1)].push(16 + r * 3 + c);
  return d;
}
...
function buildAntiDiags(): number[][] {
  const d: number[][] = Array.from({ length: 13 }, () => []);
  for (let r = 0; r < 4; r++)
    for (let c = 0; c < 4; c++) d[(3 - c) * 2 + r * 2].push(r * 4 + c);
  for (let r = 0; r < 3; r++)
    for (let c = 0; c < 3; c++)
      d[(2 - c) * 2 + 1 + (r * 2 + 1)].push(16 + r * 3 + c);
  return d;
}
...
function genDiamondGrow(): number[][] {
  ...
  for (let r = 0; r <= 6; r++) {
    const a = fill(OFF);
    for (let d = 0; d <= r; d++)
      for (const i of RINGS[d]) a[i] = d === r ? HI : MID;
    frames.push(a);
  }
  return frames;
}
...
function genSoftWaveDiag(): number[][] {
  ...
  return Array.from({ length: 13 }, (_, f) => {
    ...
    for (let d = 0; d < nD; d++) {
      ...
      const op =
        dist === 0 ? HI : dist === 1 ? MID : dist === 2 ? DIM + 0.05 : DIM;
...
function genDotMorphSoft(): number[][] {
  return Array.from({ length: 9 }, (_, f) => {
    const t = f / 8;
    return Array.from({ length: DOTS }, (_, i) =>
      i < 16 ? DIM + (HI - DIM) * (1 - t) : DIM + (HI - DIM) * t
    );
  });
}
...
function genGridMaskReveal(): number[][] {
  return Array.from({ length: 12 }, (_, f) => {
    return Array.from({ length: DOTS }, (_, i) => {
      const row =
        i < 16 ? Math.floor(i / 4) * 2 : Math.floor((i - 16) / 3) * 2 + 1;
...
function genGridSoftShift(): number[][] {
  return Array.from({ length: 15 }, (_, f) => {
    ...
    for (let col = 0; col < 7; col++) {
      ...
      const op =
        phase === 0 ? HI : phase === 1 ? MID : phase === 6 ? MID * 0.6 : DIM;
      for (const i of VCOLS[col]) a[i] = op;
    }
    return a;
  });
...
export interface MatrixSpinnerProps extends Omit<
